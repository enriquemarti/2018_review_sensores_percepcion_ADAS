...
Automated driving, however, pose some particular challenges to camera sensors
and artificial vision technology:

\begin{itemize}
    \item Varying light and visibility conditions: driving happens at day, 
    ...
    \cite{Strobel2013} presents a sensor with 130 dB range (global shutter) 
    and up to 170 dB (rolling shutter).
...
These and some other problems are analyzed in \cite{Pueo2016} from the 
perspective of recording scenes in sports.
...
Some manufacturers as Metawave \footnote{https://www.metawave.co/} 
%\cite{Metawave2018} 
are starting 
...
The first technology directs laser emitter beams within a cone using
micro-mirrors that can 
...
pattern can be random in the entire FoV (Field of View), 
%which is great for characterizing fast moving objects.
it is possible to observe only a region of interest, % within the FoV. 
and can increase point density within frames or regions for
